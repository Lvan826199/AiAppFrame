...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--奶牛小镇_182 ", *args, **kwargs)
    ...
    while True:
        try:
            ...
            if exists(pwd_field):
                ...
                time.sleep(2)
                click([0.2, 0.2])
            if exists(agree_button_182):
                touch(agree_button_182)
                print("点击账号注册的   同意协议  置为True")
                time.sleep(1)
                agree_flag = True

            if not agree_flag and exists(agree_button_182):
                touch(agree_button_182)
                print("点击设备182 账号注册的   同意协议  置为True")
                time.sleep(1)
                agree_flag = True
...
def OpenGame(c, ios):
    while True:
        ...
        if exists(game_icon) or exists(facebook_icon):
