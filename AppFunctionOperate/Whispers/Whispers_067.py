...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--Whispers_067 ", *args, **kwargs)
...
def enter_home_page():
    ...
    while True:
        if exists(WS_close_2_open):
            touch(WS_close_2_open)
            break
            ...
            if exists(WS_continue_img):
                ...
                while True:
                    if exists(WS_pause_button_img):
                        ...
                        click([0.082, 0.069])  # 点击左上角的暂停进行退出
                break
...
def login(devices, c, ios):
    ...
    while 1:
        try:
            ...
            if c.xpath('//*[@label="继续"]').exists:
                ...
                c.close()
                break
            if exists(WS_facebook_continue_067):
                touch(WS_facebook_continue_067)
                break
        except:
            traceback.print_exc()
    c.close()
    ...
    if not exists(WS_login_img):
        ...
        iosScreenshot(devices, 'Whisper登录失败')
        time.sleep(10)
