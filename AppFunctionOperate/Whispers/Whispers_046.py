...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--Whispers_046 ", *args, **kwargs)
...
def buy_charge(c, password):
    ...
    while True:
        ...
        if exists(WS_money_img_1):
            break

    while 1:
        try:
            print("状态：", c.status())
            if exists(WS_money_img_1):  # 如果存在 $ 付款按钮，则点击
                touch(WS_money_img_1)
            if c.is_ready():
                print("进行付款")
