...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--NorthTower_0067 ", *args, **kwargs)
...
def login(packageName, c, ios):
    ...
    while True:
        ...
        for i in range(5):
            ...
            # 登录AppleID
            if exists(NT_SettingButton):
                touch(NT_SettingButton)

            if exists(NT_sign):
                touch(NT_sign)
                time.sleep(1)
                c.xpath('//*[@label="继续"]').click_exists(5)
                c.xpath('//*[@label=""]').set_text("Tll123456")
                time.sleep(2)
                c.xpath('//*[@label="继续"]').click_exists(5)
                time.sleep(15)

            if exists(NT_confirm):
                touch(NT_confirm)
                print("绑定Apple ID成功")
                break
...
def pay(c, password):
    ...
    while True:
        try:
            ...
            if exists(NT_addButton_100):
                touch(NT_addButton_100)
                flag_pay = 1
            time.sleep(3)
            if exists(NT_D):
                break
    ...
    while True:
        ...
        try:
            print('--------进行充值-----------')
            if exists(NT_Money99_100):  # 如果存在 99 付款按钮，则点击
                touch(NT_Money99_100)
                time.sleep(5)
            if c.is_ready():
                print("进行付款")
    ...
    for i in range(5):
        ...
        if redHouseFlag == 3:
            ...
            click([0.075, 0.943])
        if exists(NT_redHouse_100):
            touch(NT_redHouse_100)
        if exists(NT_levelReward):
            break
