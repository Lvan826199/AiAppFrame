...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--BingoIsland_0182 ", *args, **kwargs)
...
def login(device, packageName, c, ios):
    ...
    if app == 4:
        ...
        while True:
            try:
                print("无线局域网与蜂窝网络")
                c(type='Button', name='无线局域网与蜂窝网络').click(timeout=2)
                print("点击网络接受")
                i += 1
                ...
                try:
                    if c(type='Button', name='允许').exists:
                        c(type='Button', name='允许').click(timeout=2)
                        time.sleep(0.5)
                        i += 1
    ...
    while True:
        ...
        if exists(BIS_homeHaveAnCount):
            ...
            time.sleep(2)
        # if exists(BIS_homeFacebook):
        #     touch(BIS_homeFacebook)
        #     facebook = True
        #     break
        if exists(BIS_LoginAppleID_0182):
            touch(BIS_LoginAppleID_0182)
            AppleID = True
            break
    ...
    if AppleID:
        ...
        while True:
            try:
                c(type="Button", name="使用密码继续").click()
            except:
                pass

            try:
                print("输入密码")
                c.xpath('//*[@label=""]').set_text("Tll123456")
                # c(type="SecureTextField",name="SecureTextField").set_text("Tll123456")
            except:
                pass
            ...
            try:
                ...
                c(type="Button", name="登录").click()
                time.sleep(3)
            except:
                pass

            if c(type="Button", name="允许").exists:
                print("182 ---- appleID登录成功")
                break
