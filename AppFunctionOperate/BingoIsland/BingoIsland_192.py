...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--BingoIsland_0192 ", *args, **kwargs)


def login(device, packageName, c, ios, poco_ios):
    ...
    if app == 4:
        for i in range(3):
            ...
            #     ios.alert_accept()
            try:
                poco_ios(type="Button", name="允许").click()
                time.sleep(2)
            except:
                pass
...
def pay(device, c, poco_ios, password):
    for i in range(3):
        try:
