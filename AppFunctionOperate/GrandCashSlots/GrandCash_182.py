...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--GrandCash_0182 ", *args, **kwargs)
...
def login(device, packageName, c, ios):
    ...
    while True:
        if exists(GC_gamein):
            ...
            break
    ############# 进入游戏，点击关闭叉叉
    close_list = [GC_close_red, GC_close_purple, GC_close_deepPurple, GC_close_dark, GC_close_blue, GC_close_gray, GC_close_1, GC_close_2, GC_collect]
    while True:
...
def pay(device, c, password):
    close_list = [GC_collect, GC_close_red, GC_close_purple, GC_close_deepPurple,
                  GC_close_dark, GC_close_blue, GC_close_gray, GC_close_1, GC_close_2, GC_close_3]
    while True:
...
def play_game_spin(n):
    ...
    while True:
        if exists(GC_Frog_Theme):
            ...
            swipe([0.85, 0.5], [0.4, 0.5])  # 屏幕内容朝左滚动
