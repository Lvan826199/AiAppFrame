...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--MatchScapes_0186 ", *args, **kwargs)
...
def theme_game(c, ios, n):
    ...
    ### 先完成第2关
    print("进行第二关关卡的游玩.....")
    click_list = [[0.31, 0.39], [0.5, 0.39], [0.68, 0.39],
                  [0.31, 0.37], [0.5, 0.37], [0.68, 0.37],
                  [0.31, 0.35], [0.5, 0.35], [0.68, 0.35]]
    for k in range(5):
        for j in click_list:
            click(j)
...
def theme_game_click_2():
    #### 这个线程用来点击图片
    click_list2 = [buy450, confirm, useBackGround]
    while 1:
        if avent.is_set():
            for click1 in click_list2:
                try:
