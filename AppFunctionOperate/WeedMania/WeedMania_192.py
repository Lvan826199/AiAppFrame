...
import logging
from common.WeedManiaImgs import *
...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--WeedMania_0192 ", *args, **kwargs)
...
def permission_click(c):
    ...
    for i in range(10):
        ...
        ocrScan.ocr_click(img=None, action_text={"无线局域网与蜂窝网络": 0})

        print(f"----------{i}---------")
        ocrScan.ocr_click(img=None, action_text={"允许": 0})
        time.sleep(1)
        ocrScan.ocr_click(img=None, action_text={"继续": 0})
        time.sleep(1)
        ocrScan.ocr_click(img=None, action_text={"下一步": 0})
        time.sleep(1)

        ocrScan.ocr_click(img=None, action_text={"开始测试": 0})
        time.sleep(1)

        # ocr_click(img=None, action_text={"继续": 0})
        # ocr_click(img=None, action_text={"开始": 0})
        ocrScan.ocr_click(img=None, action_text={"Accept": 1})
        time.sleep(2)
        ocrScan.ocr_click(img=None, action_text={"Continue": 0})

        if ocrScan.ocr_exist(img=None, action_text={"5/5": 0}) :
            print("进入了游戏主页面！")
            break
...
def playLevel1():
    ...
    #             click(zhuanpan_pos)
    #             time.sleep(1)
    #             ocr_click(img=None, action_text={"穿戴": 0})
    #             time.sleep(0.5)
    #             # ocr_click(img=None, action_text={"1/1": 0})
    #             ocr_click(img=None, action_text={"(": 0})
    #             time.sleep(0.5)
    #             click(zhuanpan_pos)
    #             time.sleep(0.5)
    #             if exists(upup):
    ...
    #                 time.sleep(0.5)
    #             if exists(downdown):
    #                 ocr_click(img=None, action_text={"出售": 0})
    #                 click(zhuanpan_pos)
    #                 time.sleep(0.5)
    #             time.sleep(0.5)
    #     else:break
