...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--LuckyHit_0186 ", *args, **kwargs)
...
def closeOpera(MaxNum: int = 2):
    print("进行关闭.....")
    ############ 付款成功之后需要关闭一些东西
    close_buy_list = [collect_1, close_red_1, press_to_play, Keep_STAMPING, collect_2, collect_1, close_purple_1, close_red_2, close_red_3,
                      close_purple_2,
                      close_blue_1,
                      collect_3, close_card_1, close_card_gold, collect_1, close_purple_1, play_button]
    # 支付成功之后的操作
    quitFlag = 0
    while True:
        jindu = 0
