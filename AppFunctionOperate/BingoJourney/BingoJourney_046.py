...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--BingoJourney_046 ", *args, **kwargs)
    ...
    while True:
        ...
        try:
            ...
            if exists(BJ_alert):  # 网络重新连接的一个弹窗，如果存在，则点击确定
                touch(BJ_alert)
                print("2.1")
            # if exists(BJ_re_ct): #网络重新连接的一个弹窗，如果存在，则点击确定
            #     touch(BJ_re_ct)
            #     print("2.2")
            # if exists(BJ_claim): #网络重新连接的一个弹窗，如果存在，则点击确定
            #     touch(BJ_claim)
        ...
        try:
            ...
            if facebook == True:
                if exists(BJ_game_home):
                    ...
                    print("点击facebook登录")
                iosScreenshot(devices=devices, action="journey的facebook登录")
                print("FB登录")
                if ios.alert_exists():
                    ios.alert_accept()
                    c.close()
                ...
                print("游客登录")
                iosScreenshot(devices=devices, action="journey游客登录")
...
def game_stop():
    stop = Template(r"D:\Aotoproject\BingoJourney\23728bca\tpl1624330695332.png", record_pos=(0.287, 0.038), resolution=(1920, 1080))
    while 1:
        try:
            if exists(stop) is False:
                break
        except:
            traceback.print_exc()
    avent.clear()
