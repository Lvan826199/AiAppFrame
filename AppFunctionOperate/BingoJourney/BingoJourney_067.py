...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--BingoJourney_0100 ", *args, **kwargs)
...
def break_sign(devices):
    while 1:
        ...
        if exists(BJ_hawaii):
            break
        elif exists(BJ_update_level_067):
            break
        else:
            for i in range(3):
...
def theme_game(n):
    ...
    while True:
        try:
            print("game in .....点击卡牌")
            ### 关卡的等级，多少张牌
            # click([0.407,0.46]) ##0186 0100
            click([0.632, 0.366])  ##0067
            if exists(BJ_update_level_067) is False:
                print("等待进入")
                break
            else:
                continue
...
def buy_charge(c, password):
    ...
    while 1:
        try:
            if exists(BJ_account_bt_067):  # 如果存在USD付款按钮，则点击
                touch(BJ_account_bt_067)
                print("点击购买按钮成功")
                break
        except:
            traceback.print_exc()
