...
if __name__ == "__main__":
    ...
    addr = "http://localhost:8100" # 只有连接一台设备的时候才能使用
    device_id = "00008030-001E19021A42802E" # ; 186 ->>
    driver = wda.Client("http+usbmux://{udid}".format(udid=device_id))
    # 只有一个设备的情况，可以不添加参数
    # driver = wda.USBClient()
    # # 指定设备的udid、WDA的端口号和wda_bundle_id连接
    # driver = wda.USBClient("$UDID", port=8100, wda_bundle_id="$wda_bundle_id")
    # 通过DEVICE_URL访问
    # c = wda.Client("http+usbmux://{udid}".format(udid="$UDID"))
    print(driver.status())
    ...
    screenshot = mjpeg_server.snapshot()
    print(screenshot)
    # 检查截图是否成功
    if screenshot is not None:
        # 定义保存路径
        save_path = "screenshot.png"  # 保存为当前目录下的 screenshot.png
        # 保存图片到本地
        import cv2,os
