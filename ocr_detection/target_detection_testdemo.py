...
ocr = PaddleOCR(use_gup=False, det_db_thresh=0.1, det_max_side_len=1860)


def posxy(line: List) -> Tuple:
    xmin = line[0][0][0]
    xmax = line[0][1][0]
    ymin = line[0][0][1]
    ymax = line[0][2][1]
...
def match_target(target_text, match_mode, per_ocr_text, line, rectangle, confidence, deep_res: list) -> List:
    """
...
def img_show_result(img, result):
    """
    在图像上显示结果
    :param img: 图片
...
def deep_ocr(img: Optional[Union[List, np.ndarray, str, bytes]], action_text: Dict[str, int]) -> List:
    """
    ...
    for idx in range(len(result)):
        ...
        for line in res:
            ...
            for target_text, match_mode in action_text.items():
                ...
                deep_res = match_target(target_text=target_text, match_mode=match_mode,
...
def ocr_exist(img: Optional[Union[List, np.ndarray, str, bytes]], action_text: Dict[str, int]) -> List:
    """
    返回匹配目标元素的坐标列表
    :param img:
    :param action_text:
    :return: 坐标列表
    """
    deep_res = deep_ocr(img, action_text)
    position_xy_list = []
    if deep_res:
...
def ocr_click(img: Optional[Union[List, np.ndarray, str, bytes]], action_text: Dict[str, int], order: int = 0):
    """
    ...
    # [{'0': (432, 104)}, {'0': (774, 349)}]
    try:
        position_xy_list = ocr_exist(img, action_text)
        if position_xy_list:
            if order:
...
if __name__ == '__main__':
    ...
    # // 192
    # devices = "27d62264ebf40fb3a9e4868590b62ff3b4de90ff"
