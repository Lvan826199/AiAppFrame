...
from AppFunctionOperate.DoubleRich.DoubleRich_100 import *
...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--DoubleRich_100 ", *args, **kwargs)


controlparams = {'control': 1}  # 用作控制器


class Test_DoubleRich_100(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_01_of_loginAppStore(self):
        ...
        ###########################
        # appstore 账号登录
        SH_100().loginAppStore(self.c, "xli65512@gmail.com", "Avidly1234")  # 填入用于app store登录的apple id
    ...
    def test_02_of_checkAcount(self):
        ...
        # 沙盒账号登录
        print("-------------------进行第二个测试用例---------------------")
        SH_100().check_acount_ios14(self.c, "xli65512@gmail.com", "Avidly1234")  # 填入用于app充值的沙盒账号
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        print("-------------------进行第三个测试用例---------------------")
        Device(self.device_id).app_uninstall("com.newclassic.doublerich")  # 填入自己公司app的ios包名
        ##使用testflight下载对应的app,填入的是在testFlight显示的app全程
        SH_100().game_download(self.c, name="Double Rich！Vegas Casino Slots")
        print("Double Rich！Vegas Casino Slots 下载完毕！")
        print("测试开始")
    ...
    def test_04_of_EnterApp(self):
        u'''检查进入游戏'''
        print("进入游戏部分....")
        packageName = "com.newclassic.doublerich"
        ##启动
    ...
    def test_07_of_login(self):
        ...
        stop_app("com.newclassic.doublerich")
