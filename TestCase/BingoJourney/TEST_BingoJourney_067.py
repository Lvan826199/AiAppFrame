...
from AppFunctionOperate.BingoJourney.BingoJourney_067 import *
...
class Test_BingoJourney_067(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_01_of_loginAppStore(self):
        ...
        # ###卸载
        Device(self.device_id).app_uninstall("com.bingo.scape.ios.free")  # 填入自己公司app的ios包名
        ###安装
        if self.c.locked():
            self.c.unlock()
    ...
    def test_02_of_checkAcount(self):
        ...
        # 沙盒账号登录
        print("-------------------进行第二个测试用例---------------------")
        SH_067().check_acount_ios14(self.c, "jinlimei86@gmail.com", "Avidly1234")
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        SH_067().game_download(self.c, name="Bingo Journey！Live Bingo Games")
        print("BingoJourney下载完毕！")
        print("测试开始")
    ...
    def test_04_of_loginApp(self):
        u'''检查登录功能'''
        ##启动
        self.c.app_start("com.bingo.scape.ios.free")
        sleep(10)
        ###判断app是否在运行
        app = self.ios.app_state("com.bingo.scape.ios.free")["value"]
        if app == 4:
            if self.ios.alert_exists():
                self.ios.alert_accept()
        packageName = "com.bingo.scape.ios.free"
    ...
    def test_06_of_basicFunc(self):
        u'''基础功能流程操作'''
        theme_game(n=3)
        stop_app("com.bingo.scape.ios.free")
