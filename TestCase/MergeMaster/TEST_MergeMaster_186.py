# -*- coding: utf-8 -*-
"""
@Time : 2024/10/31 18:02
@Email : Lvan826199@163.com
@公众号 : 梦无矶测开实录
@File : TEST_MergeMaster_186.py
"""
__author__ = "梦无矶小仔"
...
from AppFunctionOperate.MergeMaster.MergeMaster_186 import *
from settings.account_pwd_config import get_pwd, get_account,get_game_package
...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S - 186", time.localtime()), *args, **kwargs)


controlparams = {'control': 1}  # 用作控制器
gameName = "MergeMaster"

class Test_MergeMaster_186(ParameterizedTestCase):
    u'''测试用例MergeMaster的集合'''
    ...
    def test_01_of_loginAppStore(self):
        ...
        ###########################
        # appstore 账号登录
        account = get_account(gameName)
        pwd = get_pwd(gameName)
        SH_186().loginAppStore(self.c, account, pwd)  # 填入用于app store登录的apple id
    ...
    def test_02_of_checkAcount(self):
        ...
        account = get_account(gameName)
        pwd = get_pwd(gameName)
        SH_186().check_acount_ios14(self.c, account, pwd)  # 填入用于app充值的沙盒账号
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        print("-------------------进行第三个测试用例---------------------")
        # Device(self.device_id).app_uninstall("com.zymobile.merge.master.home.design.ios")  # 填入自己公司app的ios包名
        ##使用testflight下载对应的app,填入的是在testFlight显示的app全程
        # SH_186().game_download(self.c, name="Weed Mania: Chill 420 Club")
        print("MergeMaster 下载完毕！")
        print("测试开始")
    ...
    def test_04_of_EnterApp(self):
        u'''检查进入游戏'''
        print("进入游戏部分....")
        packageName = get_game_package(gameName)
        ##启动
        ...
        if app == 4:
            permission_click(self.c)
            print("MergeMaster游戏进入成功！")

    @unittest.skip
    def test_05_of_login(self):
        u'''账号登录及数据的保存与读取'''
        loginAppleID(self.c, self.ios, password="Tll123456")

    # @unittest.skip
    def test_06_of_PlayLevel1(self):
        u'''玩第一关'''
        player_in()
        print("第一关游玩操作完毕,进行第二关！")
        # playLevel2()
    ...
    def test_06_of_pay(self):
        ...
        pwd = get_pwd("MergeMaster")
        print(f"输入的密码：{pwd}")
        buy_charge(self.c, password=pwd)

    @unittest.skip
    def test_08_of_home(self):
        u'''关闭应用'''
        stop_app("com.zymobile.merge.master.home.design.ios")