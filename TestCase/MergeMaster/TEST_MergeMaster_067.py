...
from AppFunctionOperate.MergeMaster.MergeMaster_067 import *
...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S - 067", time.localtime()), *args, **kwargs)


controlparams = {'control': 1}  # 用作控制器
gameName = "MergeMaster"

class Test_MergeMaster_067(ParameterizedTestCase):
    ...
    def test_01_of_loginAppStore(self):
        ...
        SH_067().loginAppStore(self.c, account, pwd)  # 填入用于app store登录的apple id
    ...
    def test_02_of_checkAcount(self):
        ...
        account = get_account(gameName)
        pwd = get_pwd(gameName)
        SH_067().check_acount_ios14(self.c, account, pwd)  # 填入用于app充值的沙盒账号
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        # SH_067().game_download(self.c, name="Weed Mania: Chill 420 Club")
