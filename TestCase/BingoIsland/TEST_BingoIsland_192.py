...
from AppFunctionOperate.BingoIsland.BingoIsland_192 import *
...
class Test_BingoIsland_192(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def setUp(self):
        ...
        if controlparams['control'] == 1:
            ...
            dev = connect_device(f"iOS:///http+usbmux://{self.device_id}")
            self.poco_ios = iosPoco(device=dev)
            controlparams['ios'] = self.ios
            controlparams['c'] = self.c
            controlparams['poco_ios'] = self.poco_ios
            controlparams['control'] = 2

        # 每次执行用例前，需要从控制参数里面进行取出（但不会进行重新连接）
        self.ios = controlparams['ios']
        self.c = controlparams['c']
        self.poco_ios = controlparams["poco_ios"]
    ...
    def test_02_of_checkAcount(self):
        '''检查AppleID'''
        # 沙盒账号登录
        print("--------------0192不需要-----进行第二个测试用例---------------------")
        # SH_192().check_acount_ios14(self.c, "xli65512@gmail.com", "Tll1234567")
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        print("-------------------进行第三个测试用例---------------------")
        # ###卸载
        Device(self.device_id).app_uninstall("com.bingo.island.ios")  # 填入自己公司app的ios包名
        # 使用testflight下载对应的app,填入的是在testFlight显示的app全程
        SH_192().game_download(self.c, name="Bingo Island-Fun Family Bingo")
        print("Bingo Island-Fun Family Bingo 下载完毕！")
        print("测试开始")

    # @unittest.skip
    def test_04_of_loginApp(self):
        u'''检查登录功能'''
        packageName = "com.bingo.island.ios"
        login(self.device_id, packageName, self.c, self.ios, self.poco_ios)

    # @unittest.skip
    def test_05_of_pay(self):
        u'''充值功能检查'''
        # player_in(self.device_id)
        from settings.account_pwd_config import get_pwd
        pwd = get_pwd("BingoIsland")
        print(f"输入的密码：{pwd}")
        pay(self.device_id, self.c, password=pwd)

    # @unittest.skip
    def test_06_of_basicFunc(self):
        u'''基础功能流程操作'''
        theme_game(2)
        stop_app("com.bingo.island.ios")
