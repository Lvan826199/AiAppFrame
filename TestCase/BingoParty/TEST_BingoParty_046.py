...
from AppFunctionOperate.BingoParty.BingoParty_046 import *
...
class Test_BingoParty_046(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_01_of_loginAppStore(self):
        ...
        # ###卸载
        Device(self.device_id).app_uninstall("com.bingo.tour.party.crazy.free.ios.avidly")  # 填入自己公司app的ios包名
        ###安装
        if self.c.locked():
            self.c.unlock()
        ...
        ###########################
        # appstore 账号登录
        SH_046().check_acount(self.c, "treetll94@gmail.com", "Tll654321")  # 填入用于app store登录的apple id

    @unittest.skip
    def test_02_of_checkAcount(self):
        '''检查AppleID'''
        # 沙盒账号登录
        print("-------------------046不需要进行第二个测试用例---------------------")
        # SH_046().check_acount_ios14(self.c, "treetll94@gmail.com", "Tll654321")
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        SH_046().game_download(self.c, name="Bingo Party！Live Classic Bingo")
        print("Bingo Party！Live Classic Bingo 下载完毕！")
        print("测试开始")

    # @unittest.skip
    def test_04_of_loginApp(self):
        u'''检查登录功能'''
        packageName = "com.bingo.tour.party.crazy.free.ios.avidly"
    ...
    def test_06_of_basicFunc(self):
        u'''基础功能流程操作'''
        theme_game(2)
        stop_app("com.bingo.tour.party.crazy.free.ios.avidly")
