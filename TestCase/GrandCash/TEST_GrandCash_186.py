# -*- coding: utf-8 -*-
'''
@Time : 2022/9/21 17:35
@Author : Vincent.xiaozai
@Email : Lvan826199@163.com
@File : TEST_GrandCash_186.py.py
'''
__author__ = "梦无矶小仔"
...
from AppFunctionOperate.GrandCashSlots.GrandCash_186 import *
...
class Test_GrandCaSH_186(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_01_of_loginAppStore(self):
        ...
        # ###卸载
        Device(self.device_id).app_uninstall("com.grandcashslots.free.ios")  # 填入自己公司app的ios包名
        ###安装
        if self.c.locked():
            self.c.unlock()
    ...
    @unittest.skip
    def test_03_of_Download(self):
        u'''进入TestFlight进行下载App'''
        print("-------------------进行第三个测试用例---------------------")
        ##使用testflight下载对应的app,填入的是在testFlight显示的app全程
        SH_186().game_download(self.c, name="Grand Cash Casino Slots Games")
        print("Grand Cash Casino Slots Games 下载完毕！")
        print("测试开始")

    # @unittest.skip
    def test_04_of_loginApp(self):
        u'''检查登录功能'''
        packageName = "com.grandcashslots.free.ios"
        login(self.device_id, packageName, self.c, self.ios)

    # @unittest.skip
    def test_05_of_pay(self):
        u'''充值功能检查'''
        from settings.account_pwd_config import get_pwd
        pwd = get_pwd("GrandCash")
        print(f"输入的密码：{pwd}")
    ...
    def test_06_of_basicFunc(self):
        u'''基础功能流程操作'''
        play_game_spin(n=3)
        stop_app("com.grandcashslots.free.ios")
