...
from AppFunctionOperate.CashHoard.CashHoard_067 import *
...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--CashHoard_067 ", *args, **kwargs)


controlparams = {'control': 1}  # 用作控制器


class Test_CashHoard_067(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_03_of_DownloadApp(self):
        ...
        print("-------------------进行第三个测试用例---------------------")
        Device(self.device_id).app_uninstall("com.jackpot.fever.slots.ios")  # 填入自己公司app的ios包名
        ##使用testflight下载对应的app,填入的是在testFlight显示的app全程
        SH_067().game_download(self.c, name="Cash Hoard Casino Slots Games")
        print("Cash Hoard Casino Slots Games 下载完毕！")
        print("测试开始")
    ...
    def test_04_of_EnterApp(self):
        u'''检查进入游戏'''
        print("进入游戏部分....")
        packageName = "com.jackpot.fever.slots.ios"
        ##启动
    ...
    def test_07_of_otherLevel(self):
        ...
        stop_app("com.jackpot.fever.slots.ios")
