...
from AppFunctionOperate.HugeWin.HugeWin_067 import *
...
def print(*args, **kwargs):
    _print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "--HugWin_067 ", *args, **kwargs)


controlparams = {'control': 1}  # 用作控制器


class Test_HugWin_067(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_01_of_loginAppStore(self):
        ...
        ###########################
        # appstore 账号登录
        SH_067().loginAppStore(self.c, "jinlimei86@gmail.com", "Avidly1234")  # 填入用于app store登录的apple id
    ...
    def test_02_of_checkAcount(self):
        ...
        # 沙盒账号登录
        print("-------------------进行第二个测试用例---------------------")
        SH_067().check_acount_ios14(self.c, "jinlimei86@gmail.com", "Avidly1234")  # 填入用于app充值的沙盒账号
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        print("-------------------进行第三个测试用例---------------------")
        Device(self.device_id).app_uninstall("com.cjfafafa.trojan.ios")  # 填入自己公司app的ios包名
        ##使用testflight下载对应的app,填入的是在testFlight显示的app全程
        SH_067().game_download(self.c, name="Huge Win Slots！Casino Games")
        print("Huge Win Slots！Casino Games 下载完毕！")
        print("测试开始")
    ...
    def test_04_of_EnterApp(self):
        u'''检查进入游戏'''
        print("进入游戏部分....")
        packageName = "com.cjfafafa.trojan.ios"
        ##启动
        ...
        time.sleep(10)
        enterGame(self.c, self.ios)
        print("Huge Win Slots！游戏进入成功！")
    ...
    def test_07_of_login(self):
        ...
        stop_app("com.cjfafafa.trojan.ios")
