...
from AppFunctionOperate.NorthTower.NorthTower_192 import *
...
class Test_NorthTower_192(ParameterizedTestCase):
    u'''测试用例Demo的集合'''
    ...
    def test_02_of_checkAcount(self):
        ...
        # SH_192().check_acount_ios14(self.c, "xli65512@gmail.com", "Avidly1234")
        ###################################
    ...
    def test_03_of_DownloadApp(self):
        ...
        print("-------------------进行第三个测试用例---------------------")
        # ###卸载
        Device(self.device_id).app_uninstall("com.strategy.north.tower.ios")  # 填入自己公司app的ios包名
        ##使用testflight下载对应的app,填入的是在testFlight显示的app全程
        SH_192().game_download(self.c, name="North Tower! Merge TD Defense")
        print("North Tower! Merge TD Defense 下载完毕！")
        print("测试开始")

    # @unittest.skip
    def test_04_of_loginApp(self):
        u'''检查登录功能'''
        packageName = "com.strategy.north.tower.ios"
        login(packageName, self.c, self.ios)
    ...
    def test_05_of_pay(self):
        ...
        # pwd = get_pwd("NorthTower")
        # print(f"输入的密码：{pwd}")
        # pay(self.c, password='Tll1234567')
        print("0192-跳过重置")
    ...
    def test_06_of_basicFunc(self):
        u'''基础功能流程操作'''
        # theme_game(2)
        stop_app("com.strategy.north.tower.ios")
